...
Simulation collection and simulation creation. / Discovered from platform frontend bundle.

- Advertised methods: GET, OPTIONS, POST.
- Live OPTIONS status: 200. Mutation methods were not executed.
- Account-specific option values are omitted; use `wqb api call` with OPTIONS for current metadata.
- ALL uses a single REGION_AGNOSTIC parent request. It cannot appear in an HTTP batch array; sqlitesimu can interleave it with Regular batches.
