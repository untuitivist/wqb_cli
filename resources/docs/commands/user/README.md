...
Use `wqb api show PATH` to inspect live-probe evidence and `wqb api call PATH --method OPTIONS` for fresh server metadata. Some endpoints reject OPTIONS even while their Allow headers advertise methods. Advertised mutation methods are not a guarantee of account permission.
