...
set "NODE_TMP=%TEMP%\codex_node_dir_F.txt"

if "%~5"=="" (
  "%PYTHON_EXE%" "%ROOT_DIR%\workflow\shared\resolve_node_dir.py" "%RUN_DIR%" "F_community_help_center_experience" create > "%NODE_TMP%"
...
"%PYTHON_EXE%" "%SCRIPT_DIR%scripts\collect_community_experience.py" "%RUN_DIR%" "%NODE_DIR%" "%REGION%" "%DELAY%" "%CATEGORY%"
if errorlevel 1 exit /b 1

"%PYTHON_EXE%" "%SCRIPT_DIR%scripts\write_summary.py" "%NODE_DIR%" "%REGION%" "%DELAY%" "%CATEGORY%"
if errorlevel 1 exit /b 1

echo Node completed: community_and_help_center_experience
