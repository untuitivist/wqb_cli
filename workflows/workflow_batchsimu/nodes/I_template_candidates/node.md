...
按 F 的固定 seed 和配额，从 H 定义的每个有效 family population 中无放回抽样，生成完整 lineage、唯一 simulation identity 和可直接交给 `wqb sqlitesimu enqueue` 的 manifest。
...
- `workflow_run_id`
- `template_family_id`、`template_version`、`family_draw_index`
- `mechanism_id`、`component_ids`、字段 id 与字段角色
- 完整参数选择、rng seed、population ordinal
- canonical expression、expression hash、settings hash
...
- `validation_report.json.verdict = true` 且 violation count 全为 0。
