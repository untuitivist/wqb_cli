...
4. `BLOCKED`、`CANCELLED`、`SIMULATE_UNKNOWN` 或 READY coverage 不足时，先把 run 标为 `analysis_ineligible`；固定三段报告仍可输出，但只能描述，不得选择扩展 family。
...
在上述统计前，必须按 `template_contract.md` 生成固定的三段表和逐模板 `实验成果评估 / 关键发现 / 改进方向`。代表 alpha 使用有符号最大 Sharpe/Fitness；无 `FAIL/ERROR` 的候选只能在各自 template 内筛选和 fallback。没有 READY 的模板也必须出现在报告中。
...
- 固定报告、预注册统计和最终 eligibility 分层保存，固定报告不能单独触发扩展。
