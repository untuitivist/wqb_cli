...
- Never schedule a new node while `graph_state.current_node` is non-null.
...
- Do not write node files other than `node_input.json` and the empty `outputs/` directory created by `create_node_task.py`.
- Do not write run-level summaries while a graph is still active. Summaries are allowed only after graph completion or block.
...
Reject a node result if any required bundle file is missing or invalid.
If `process_log.md` and `validation_report.json` are still missing shortly after assignment, treat the nodesubagent as non-compliant and redo the node in a new node directory.
...
3. Validate the node bundle with `workgraph/regular/scripts/validate_node_bundle.py`.
4. Update `graph_state.json` with `workgraph/regular/scripts/update_graph_state.py`.
5. Decide the next node from `allowed_next`.
