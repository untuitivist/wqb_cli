"""
功能概述
`wqb_core.alpha.filter_alphas` 模块。
...
class FilterAlphasMixin:
    def filter_alphas(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.filter_alphas(...)` 暴露。
        """
        if log is None:
            log_gap = 0
        ...
        total = len(offsets)
        for idx, page_offset in enumerate(offsets, start=1):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(FilterAlphasMixin.filter_alphas) or "")
    ...
    target = getattr(session, "filter_alphas")
