"""
功能概述
`wqb_core.alpha.get_alpha_recordset_data` 模块。

这个文件提供与当前文件名对应的具体实现。

主推荐入口
- `get_alpha_recordset_data(...)`
...
from ..foundation.urls import URL_ALPHAS_ALPHAID_RECORDSETS


class GetAlphaRecordsetDataMixin:
    def get_alpha_recordset_data(
        self,
        alpha_id: str,
        recordset_name: str,
        *args,
        ...
        **kwargs,
        ...
        """
        获取 Alpha recordset 明细数据。
        
        该方法通过 `WQBSession.get_alpha_recordset_data(...)` 暴露。
        """
        url = URL_ALPHAS_ALPHAID_RECORDSETS.format(alpha_id) + f'/{recordset_name}'
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join(
                    (f"{self}.get_alpha_recordset_data(...) [", f"    {url}", f"]: {log}")
                )
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetAlphaRecordsetDataMixin.get_alpha_recordset_data) or "")
    ...
    target = getattr(session, "get_alpha_recordset_data")
