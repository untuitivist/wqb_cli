"""
功能概述
`wqb_core.alpha.get_alpha_recordsets` 模块。
...
class GetAlphaRecordsetsMixin:
    def get_alpha_recordsets(
        self,
        alpha_id: str,
        *args,
        log: str | None = '',
        **kwargs,
        ...
        """
        获取 Alpha 的 recordsets 列表。
        
        该方法通过 `WQBSession.get_alpha_recordsets(...)` 暴露。
        """
        url = URL_ALPHAS_ALPHAID_RECORDSETS.format(alpha_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.get_alpha_recordsets(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetAlphaRecordsetsMixin.get_alpha_recordsets) or "")
    ...
    target = getattr(session, "get_alpha_recordsets")
