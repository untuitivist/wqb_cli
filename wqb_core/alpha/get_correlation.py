"""
功能概述
`wqb_core.alpha.get_correlation` 模块。
...
class GetCorrelationMixin:
    _CORRELATION_GETTERS = {
        'prod': 'get_correlation_product',
        'product': 'get_correlation_product',
        'pc': 'get_correlation_product',
        'self': 'get_correlation_self',
        'sc': 'get_correlation_self',
        'powerpool': 'get_correlation_powerpool',
        'power_pool': 'get_correlation_powerpool',
        'ppc': 'get_correlation_powerpool',
    }
    ...
    def get_correlation(
        ...
        **kwargs,
    ) -> dict[str, Any] | tuple[dict[str, Any] | None, int | None, float | None] | None:
        """
        按类型统一获取相关性结果。
        
        该方法通过 `WQBSession.get_correlation(...)` 暴露。
        """
        key = correlation_type.strip().lower()
        try:
            getter_name = self._CORRELATION_GETTERS[key]
        except KeyError as exc:
            supported = ', '.join(sorted(self._CORRELATION_GETTERS))
            raise ValueError(
                f"Unsupported correlation_type={correlation_type!r}. Supported values: {supported}"
            ) from exc
        getter = getattr(self, getter_name)
        return getter(alpha_id, **kwargs)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetCorrelationMixin.get_correlation) or "")
    ...
    target = getattr(session, "get_correlation")
