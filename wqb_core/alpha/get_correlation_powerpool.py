"""
功能概述
`wqb_core.alpha.get_correlation_powerpool` 模块。

这个文件提供与当前文件名对应的具体实现。

主推荐入口
- `get_correlation_powerpool(...)`
...
from ..foundation.urls import URL_ALPHAS_ALPHAID_CORRELATION_POWERPOOL
from ._common import WaitForJsonMixin


class GetCorrelationPowerpoolMixin(WaitForJsonMixin):
    def _get_correlation_powerpool_raw(
        self,
        alpha_id: str,
        log: str | None = '',
        **kwargs,
    ) -> Response:
        url = URL_ALPHAS_ALPHAID_CORRELATION_POWERPOOL.format(alpha_id)
        resp = self.get(url, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}._get_correlation_powerpool_raw(...) [", f"    {url}", f"]: {log}"))
            )
        return resp

    def get_correlation_powerpool(
        self,
        alpha_id: str,
        ...
        该方法通过 `WQBSession.get_correlation_powerpool(...)` 暴露。
        """
        return self._wait_for_json(
            self._get_correlation_powerpool_raw,
            alpha_id,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetCorrelationPowerpoolMixin.get_correlation_powerpool) or "")
    ...
    target = getattr(session, "get_correlation_powerpool")
