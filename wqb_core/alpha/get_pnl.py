...
from ..foundation.urls import URL_ALPHAS_ALPHAID_RECORDSETS_PNL
from ._common import WaitForJsonMixin


class GetPnlMixin(WaitForJsonMixin):
    def _get_pnl_raw(
        self,
        alpha_id: str,
        log: str | None = '',
        **kwargs,
        ...
        url = URL_ALPHAS_ALPHAID_RECORDSETS_PNL.format(alpha_id)
        resp = self.get(url, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}._get_pnl_raw(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetPnlMixin.get_pnl) or "")
    ...
    target = getattr(session, "get_pnl")
