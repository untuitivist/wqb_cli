"""
功能概述
`wqb_core.alpha.get_submission_check` 模块。
...
class GetSubmissionCheckMixin:
    def get_submission_check(
        self,
        alpha_id: str,
        *args,
        log: str | None = '',
        **kwargs,
        ...
        该方法通过 `WQBSession.get_submission_check(...)` 暴露。
        """
        correlation_checks = self.get_correlation(alpha_id, *args, log=None, **kwargs)
        alpha_details_resp = self.locate_alpha(alpha_id, *args, log=None, **kwargs)
        alpha_details_resp.raise_for_status()
        if isinstance(correlation_checks, tuple):
            correlation_payload = correlation_checks[0]
        else:
            correlation_payload = correlation_checks
        all_passed = (
            correlation_payload.get('all_passed')
            if isinstance(correlation_payload, dict)
            else None
        )
        checks = {
            'correlation_checks': correlation_checks,
            'alpha_details': alpha_details_resp.json(),
            'all_passed': all_passed,
        }
        if log is not None:
            self.logger.info(f"{self}.get_submission_check(...): {log}")
        return checks
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetSubmissionCheckMixin.get_submission_check) or "")
    ...
    target = getattr(session, "get_submission_check")
