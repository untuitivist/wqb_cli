"""
功能概述
`wqb_core.alpha.locate_alpha` 模块。
...
class LocateAlphaMixin:
    def locate_alpha(
        ...
        **kwargs,
        ...
        """
        定位并获取单个 Alpha 的原始详情响应。
        
        该方法通过 `WQBSession.locate_alpha(...)` 暴露。
        """
        url = URL_ALPHAS_ALPHAID.format(alpha_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.locate_alpha(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(LocateAlphaMixin.locate_alpha) or "")
    ...
    target = getattr(session, "locate_alpha")
