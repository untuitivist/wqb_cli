"""
功能概述
`wqb_core.alpha.performance_comparison` 模块。
...
from ..foundation.urls import URL_ALPHAS_ALPHAID_PERFORMANCE_COMPARISON


class PerformanceComparisonMixin:
    def performance_comparison(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.performance_comparison(...)` 暴露。
        """
        url = URL_ALPHAS_ALPHAID_PERFORMANCE_COMPARISON.format(alpha_id)
        params = {k: v for k, v in {'teamId': team_id, 'competition': competition}.items() if v is not None}
        resp = self.get(url, *args, params=params, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.performance_comparison(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(PerformanceComparisonMixin.performance_comparison) or "")
    ...
    target = getattr(session, "performance_comparison")
