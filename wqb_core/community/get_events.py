"""
功能概述
`wqb_core.community.get_events` 模块。
...
from ..foundation.urls import URL_EVENTS
...
class GetEventsMixin:
    def get_events(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_events(...)` 暴露。
        """
        url = URL_EVENTS
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_events(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetEventsMixin.get_events) or "")
    ...
    target = getattr(session, "get_events")
