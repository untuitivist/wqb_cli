"""
功能概述
`wqb_core.community.get_leaderboard` 模块。
...
if __name__ == "__main__" and (__package__ is None or __package__ == ""):
    ...
    __package__ = "wqb_core.community"

from requests import Response

from ..foundation.urls import URL_CONSULTANT_BOARDS_LEADER, URL_USERS_SELF


class GetLeaderboardMixin:
    def get_leaderboard(
        self,
        *args,
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_leaderboard(...)` 暴露。
        """
        url = URL_CONSULTANT_BOARDS_LEADER
        params = {}
        if user_id is not None:
            ...
            params['user'] = user_resp.json().get('id')
        resp = self.get(url, *args, params=params, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_leaderboard(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetLeaderboardMixin.get_leaderboard) or "")
    ...
    target = getattr(session, "get_leaderboard")
