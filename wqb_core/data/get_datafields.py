"""
功能概述
`wqb_core.data.get_datafields` 模块。
...
class GetDatafieldsMixin(SearchFieldsLimitedMixin):
    def get_datafields(
        self,
        *args,
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_datafields(...)` 暴露。
        """
        theme_bool = theme if isinstance(theme, bool) else str(theme).lower() == 'true'
        type_value = None if data_type in ('', 'ALL', None) else data_type
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetDatafieldsMixin.get_datafields) or "")
    ...
    target = getattr(session, "get_datafields")
