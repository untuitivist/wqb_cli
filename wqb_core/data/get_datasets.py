...
class GetDatasetsMixin(SearchDatasetsLimitedMixin):
    def get_datasets(
        self,
        *args,
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_datasets(...)` 暴露。
        """
        theme_bool = theme if isinstance(theme, bool) else str(theme).lower() == 'true'
        return self.search_datasets_limited(
            region,
            delay,
            universe,
            *args,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetDatasetsMixin.get_datasets) or "")
    ...
    target = getattr(session, "get_datasets")
