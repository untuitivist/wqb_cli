"""
功能概述
`wqb_core.data.locate_dataset` 模块。
...
from ..foundation.urls import URL_DATASETS_DATASETID


class LocateDatasetMixin:
    def locate_dataset(
        self,
        dataset_id: str,
        *args,
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.locate_dataset(...)` 暴露。
        """
        url = URL_DATASETS_DATASETID.format(dataset_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.locate_dataset(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(LocateDatasetMixin.locate_dataset) or "")
    ...
    target = getattr(session, "locate_dataset")
