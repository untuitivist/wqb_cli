...
from ..foundation.urls import URL_DATAFIELDS_FIELDID


class LocateFieldMixin:
    def locate_field(
        self,
        field_id: str,
        *args,
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.locate_field(...)` 暴露。
        """
        url = URL_DATAFIELDS_FIELDID.format(field_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.locate_field(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(LocateFieldMixin.locate_field) or "")
    ...
    target = getattr(session, "locate_field")
