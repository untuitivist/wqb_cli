"""
功能概述
`wqb_core.data.run_selection` 模块。
...
from ..foundation.defines import EQUITY
from ..foundation.urls import URL_SIMULATIONS_SUPER_SELECTION


class RunSelectionMixin:
    def run_selection(
        self,
        selection: str,
        *args,
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.run_selection(...)` 暴露。
        """
        url = URL_SIMULATIONS_SUPER_SELECTION
        params = {
            'selection': selection,
        ...
        if log is not None:
            self.logger.info('\n'.join((f"{self}.run_selection(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(RunSelectionMixin.run_selection) or "")
    ...
    target = getattr(session, "run_selection")
