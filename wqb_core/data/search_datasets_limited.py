"""
功能概述
`wqb_core.data.search_datasets_limited` 模块。
...
from ..foundation.urls import URL_DATASETS


class SearchDatasetsLimitedMixin:
    def search_datasets_limited(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.search_datasets_limited(...)` 暴露。
        """
        ...
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.search_datasets_limited(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(SearchDatasetsLimitedMixin.search_datasets_limited) or "")
    ...
    target = getattr(session, "search_datasets_limited")
