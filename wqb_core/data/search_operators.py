"""
功能概述
`wqb_core.data.search_operators` 模块。
...
from ..foundation.urls import URL_OPERATORS


class SearchOperatorsMixin:
    def search_operators(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.search_operators(...)` 暴露。
        """
        url = URL_OPERATORS
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.search_operators(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(SearchOperatorsMixin.search_operators) or "")
    ...
    target = getattr(session, "search_operators")
