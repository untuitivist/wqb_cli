"""
功能概述
`wqb_core.simulation.concurrent_check` 模块。
...
class ConcurrentCheckMixin:
    async def concurrent_check(
        ...
        **kwargs,
        ...
        """
        并发轮询多个 Alpha 的 check 结果。
        
        该方法通过 `WQBSession.concurrent_check(...)` 暴露。
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(ConcurrentCheckMixin.concurrent_check) or "")
    ...
    target = getattr(session, "concurrent_check")
