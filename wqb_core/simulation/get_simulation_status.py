"""
功能概述
`wqb_core.simulation.get_simulation_status` 模块。
...
class GetSimulationStatusMixin:
    def get_simulation_status(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_simulation_status(...)` 暴露。
        """
        resp = self.get(simulation_url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join(
                    (f"{self}.get_simulation_status(...) [", f"    {simulation_url}", f"]: {log}")
                )
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetSimulationStatusMixin.get_simulation_status) or "")
    ...
    target = getattr(session, "get_simulation_status")
