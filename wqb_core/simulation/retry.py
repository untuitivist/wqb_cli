...
from ..foundation.defines import RETRY_AFTER
...
class RetryMixin:
    async def retry(
        ...
        **kwargs,
        ...
        """
        重复请求某个 endpoint，直到满足停止条件或达到重试上限。
        
        该方法通过 `WQBSession.retry(...)` 暴露。
        ...
        for tries, _ in enumerate(max_tries, start=1):
            ...
            try:
                ...
                if max_key_errors <= key_errors:
                    if log is not None:
                        self.logger.info(f"{self}.retry(...) [{key_errors} key_errors]: {log}")
                ...
                if max_value_errors <= value_errors:
                    if log is not None:
                        self.logger.info(f"{self}.retry(...) [{value_errors} value_errors]: {log}")
                    if on_success is not None:
                        on_success(locals())
                    break
            ...
            if resp is not None and resp.status_code != 429:
                ...
                self.logger.warning(
                    '\n'.join(
                        (
                            f"{self}.simulate(...) [max tries exhausted]:",
                            f"{resp}:",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(RetryMixin.retry) or "")
