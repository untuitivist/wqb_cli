...
from ..foundation.defines import POST
from ..foundation.urls import URL_ALPHAS_ALPHAID_SUBMIT
...
class SubmitMixin:
    async def submit(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.submit(...)` 暴露。
        
        这是一个异步方法；若直接在脚本中运行，入口会自动处理事件循环。
        """
        url = URL_ALPHAS_ALPHAID_SUBMIT.format(alpha_id)
        resp = await self.retry(POST, url, *args, max_tries=max_tries, log=retry_log, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.submit(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(SubmitMixin.submit) or "")
