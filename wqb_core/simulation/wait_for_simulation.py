"""
功能概述
`wqb_core.simulation.wait_for_simulation` 模块。
...
class WaitForSimulationMixin:
    async def wait_for_simulation(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.wait_for_simulation(...)` 暴露。
        ...
        while loop.time() < deadline:
            resp = self.get_simulation_status(simulation_url, *args, log=None, **kwargs)
            retry_after = resp.headers.get('Retry-After')
            if not retry_after:
                break
            await asyncio.sleep(max(float(retry_after), poll_interval))
        if log is not None:
            self.logger.info(
                '\n'.join(
                    (f"{self}.wait_for_simulation(...) [", f"    {simulation_url}", f"]: {log}")
                )
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(WaitForSimulationMixin.wait_for_simulation) or "")
    ...
    target = getattr(session, "wait_for_simulation")
