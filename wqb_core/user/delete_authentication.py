"""
功能概述
`wqb_core.user.delete_authentication` 模块。
...
from ..foundation.urls import URL_AUTHENTICATION


class DeleteAuthenticationMixin:
    def delete_authentication(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.delete_authentication(...)` 暴露。
        """
        url = URL_AUTHENTICATION
        resp = self.delete(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.delete_authentication(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(DeleteAuthenticationMixin.delete_authentication) or "")
    ...
    target = getattr(session, "delete_authentication")
