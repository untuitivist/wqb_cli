"""
功能概述
`wqb_core.user.get_authentication` 模块。
...
class GetAuthenticationMixin:
    def get_authentication(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_authentication(...)` 暴露。
        """
        url = URL_AUTHENTICATION
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.get_authentication(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetAuthenticationMixin.get_authentication) or "")
    ...
    target = getattr(session, "get_authentication")
