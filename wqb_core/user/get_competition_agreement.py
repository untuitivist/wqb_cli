"""
功能概述
`wqb_core.user.get_competition_agreement` 模块。
...
from ..foundation.urls import URL_COMPETITIONS_COMPETITIONID_AGREEMENT


class GetCompetitionAgreementMixin:
    def get_competition_agreement(
        self,
        competition_id: str,
        *args,
        log: str | None = '',
        **kwargs,
        ...
        """
        获取 competition agreement。
        
        该方法通过 `WQBSession.get_competition_agreement(...)` 暴露。
        """
        url = URL_COMPETITIONS_COMPETITIONID_AGREEMENT.format(competition_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_competition_agreement(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetCompetitionAgreementMixin.get_competition_agreement) or "")
    ...
    target = getattr(session, "get_competition_agreement")
