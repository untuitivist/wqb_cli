"""
功能概述
`wqb_core.user.get_competition_details` 模块。
...
from ..foundation.urls import URL_COMPETITIONS_COMPETITIONID


class GetCompetitionDetailsMixin:
    def get_competition_details(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_competition_details(...)` 暴露。
        """
        url = URL_COMPETITIONS_COMPETITIONID.format(competition_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_competition_details(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetCompetitionDetailsMixin.get_competition_details) or "")
    ...
    target = getattr(session, "get_competition_details")
