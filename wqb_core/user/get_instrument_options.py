"""
功能概述
`wqb_core.user.get_instrument_options` 模块。
...
class GetInstrumentOptionsMixin:
    def get_instrument_options(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_instrument_options(...)` 暴露。
        """
        return self.get_platform_setting_options(*args, log=log, **kwargs)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetInstrumentOptionsMixin.get_instrument_options) or "")
    ...
    target = getattr(session, "get_instrument_options")
