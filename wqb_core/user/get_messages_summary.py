"""
功能概述
`wqb_core.user.get_messages_summary` 模块。
...
class GetMessagesSummaryMixin:
    def get_messages_summary(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_messages_summary(...)` 暴露。
        """
        data = self.get_messages(*args, limit=limit, offset=offset, log=None, **kwargs)
            ...
            summary.append(
                {
                    ...
                    'read': msg.get('read'),
                    'sanitized': msg.get('sanitized', False),
                }
            )
        if log is not None:
            self.logger.info(f"{self}.get_messages_summary(...): {log}")
        return {
            'count': len(summary),
            'results': summary,
            'image_handling': data.get('image_handling'),
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetMessagesSummaryMixin.get_messages_summary) or "")
    ...
    target = getattr(session, "get_messages_summary")
