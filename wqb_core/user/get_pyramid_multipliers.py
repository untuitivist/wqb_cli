"""
功能概述
`wqb_core.user.get_pyramid_multipliers` 模块。
...
from ..foundation.urls import URL_USERS_SELF_ACTIVITIES_PYRAMID_MULTIPLIERS


class GetPyramidMultipliersMixin:
    def get_pyramid_multipliers(
        self,
        *args,
        log: str | None = '',
        **kwargs,
        ...
        该方法通过 `WQBSession.get_pyramid_multipliers(...)` 暴露。
        """
        url = URL_USERS_SELF_ACTIVITIES_PYRAMID_MULTIPLIERS
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join(
                    (
                        f"{self}.get_pyramid_multipliers(...) [",
                        f"    {url}",
                        f"]: {log}",
                    )
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetPyramidMultipliersMixin.get_pyramid_multipliers) or "")
    ...
    target = getattr(session, "get_pyramid_multipliers")
