"""
功能概述
`wqb_core.user.get_tutorial_page` 模块。
...
from ..foundation.urls import URL_TUTORIAL_PAGES_PAGEID


class GetTutorialPageMixin:
    def get_tutorial_page(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_tutorial_page(...)` 暴露。
        """
        url = URL_TUTORIAL_PAGES_PAGEID.format(page_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_tutorial_page(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetTutorialPageMixin.get_tutorial_page) or "")
    ...
    target = getattr(session, "get_tutorial_page")
