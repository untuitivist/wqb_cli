"""
功能概述
`wqb_core.user.get_tutorials` 模块。
...
from ..foundation.urls import URL_TUTORIALS


class GetTutorialsMixin:
    def get_tutorials(
        self,
        *args,
        log: str | None = '',
        **kwargs,
        ...
        该方法通过 `WQBSession.get_tutorials(...)` 暴露。
        """
        url = URL_TUTORIALS
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_tutorials(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetTutorialsMixin.get_tutorials) or "")
    ...
    target = getattr(session, "get_tutorials")
