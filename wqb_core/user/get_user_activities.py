"""
功能概述
`wqb_core.user.get_user_activities` 模块。
...
from ..foundation.urls import URL_USERS_USERID_ACTIVITIES


class GetUserActivitiesMixin:
    def get_user_activities(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_user_activities(...)` 暴露。
        """
        url = URL_USERS_USERID_ACTIVITIES.format(user_id)
        params = {}
        if grouping is not None:
            params['grouping'] = grouping
        resp = self.get(url, *args, params=params, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_user_activities(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetUserActivitiesMixin.get_user_activities) or "")
    ...
    target = getattr(session, "get_user_activities")
