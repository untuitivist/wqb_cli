"""
功能概述
`wqb_core.user.get_user_alphas` 模块。
...
class GetUserAlphasMixin:
    def get_user_alphas(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_user_alphas(...)` 暴露。
        """
        url = URL_USERS_SELF_ALPHAS
        params = {
            'stage': stage,
            'limit': limit,
            'offset': offset,
        ...
        if submission_start_date is not None:
            params['dateSubmitted>'] = submission_start_date
        if submission_end_date is not None:
        ...
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_user_alphas(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetUserAlphasMixin.get_user_alphas) or "")
    ...
    target = getattr(session, "get_user_alphas")
