"""
功能概述
`wqb_core.user.get_user_competitions` 模块。
...
from ..foundation.urls import URL_USERS_SELF, URL_USERS_USERID_COMPETITIONS


class GetUserCompetitionsMixin:
    def get_user_competitions(
        ...
        **kwargs,
        ...
        """
        获取用户参与的 competitions。
        
        该方法通过 `WQBSession.get_user_competitions(...)` 暴露。
        """
        resolved_user_id = user_id
        if resolved_user_id is None:
            user_resp = self.get(URL_USERS_SELF, *args, **kwargs)
            if user_resp.status_code == 200:
                resolved_user_id = user_resp.json().get('id')
            else:
                resolved_user_id = 'self'
        url = URL_USERS_USERID_COMPETITIONS.format(resolved_user_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_user_competitions(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetUserCompetitionsMixin.get_user_competitions) or "")
    ...
    target = getattr(session, "get_user_competitions")
