"""
功能概述
`wqb_core.user.get_user_profile` 模块。
...
from ..foundation.urls import URL_USERS_USERID


class GetUserProfileMixin:
    def get_user_profile(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.get_user_profile(...)` 暴露。
        """
        url = URL_USERS_USERID.format(user_id)
        resp = self.get(url, *args, **kwargs)
        if log is not None:
            self.logger.info('\n'.join((f"{self}.get_user_profile(...) [", f"    {url}", f"]: {log}")))
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(GetUserProfileMixin.get_user_profile) or "")
    ...
    target = getattr(session, "get_user_profile")
