"""
功能概述
`wqb_core.user.head_authentication` 模块。
...
class HeadAuthenticationMixin:
    def head_authentication(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.head_authentication(...)` 暴露。
        """
        url = URL_AUTHENTICATION
        resp = self.head(url, *args, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.head_authentication(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(HeadAuthenticationMixin.head_authentication) or "")
    ...
    target = getattr(session, "head_authentication")
