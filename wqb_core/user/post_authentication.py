"""
功能概述
`wqb_core.user.post_authentication` 模块。
...
class PostAuthenticationMixin:
    def post_authentication(
        ...
        **kwargs,
        ...
        该方法通过 `WQBSession.post_authentication(...)` 暴露。
        """
        url = URL_AUTHENTICATION
        resp = self.post(url, *args, auth=self.wqb_auth, **kwargs)
        if log is not None:
            self.logger.info(
                '\n'.join((f"{self}.post_authentication(...) [", f"    {url}", f"]: {log}"))
            )
        return resp
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=inspect.getdoc(PostAuthenticationMixin.post_authentication) or "")
    ...
    target = getattr(session, "post_authentication")
